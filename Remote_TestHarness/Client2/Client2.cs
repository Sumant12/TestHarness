    ...
    public class Client2 : IClient
    {
        ...
        void ThreadProc()
        {
            while (true)
            {
                ...
                Console.WriteLine("Total Time of Execution of Test Request is:");
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("==========================");
            Console.WriteLine("Starting of Client");
            Console.WriteLine("==========================");
            try
            {
                Client2 client2 = new Client2();

                string ClientStream = "http://localhost:8080/Client2IStreamService";
                string RepoStream = "http://localhost:8080/RepoIService";
                string THStream = "http://localhost:8080/TestHarnessIService";
                client2.CreateClientRecvChannel(ClientStream);
                ...
                string filepath = Path.GetFullPath(client2.ToSendPath);
                ...
                foreach (string file in files)
                {
                    ...
                    client2.uploadFile(filename, RepoStream);
                }
                Thread.Sleep(10000);
                Console.WriteLine();
                Console.WriteLine("Buidling the Message which is sent to Test Harness");
                Console.WriteLine();
                Message msg = client2.buildTestMessage();
                ...
                client2.makeQuery("test1", ClientStream);
            }
            catch (Exception ex)
            {
                Console.Write("\n\n  {0}\n\n", ex.Message);
